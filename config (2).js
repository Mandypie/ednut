...
if (!Array.isArray(premium)) throw new Error("The premium.json file must be an Owner!");
} catch (err) {
console.error("⚠️ Failed to read premium.json, using empty Box.");
premium = [];
}
const addPremiumUser = (userId, expired, _dir = premium) => {
if (!Array.isArray(_dir)) {
console.error("❌ ERROR: _This user not valid!");
return false;
}
